...
import { MUTED_OLIVE, BG, CHARCOAL } from "@/utils/constants";
...
const APP_ACCENT = 'var(--app-accent)';
const APP_ICON_MUTED = 'var(--app-icon-muted)';

const UserAvatar = ({ src, firstName, size = 36, border = `2px solid ${APP_ACCENT}`, bg = APP_ACCENT }) => (
...
const handleLogout = async () => {
  ...
  return (
    <AppShell
      ...
      padding={0}
    >
      <AppShell.Navbar style={{ backgroundColor: 'var(--app-surface)', borderRight: '1px solid var(--app-border)', boxShadow: '2px 0 8px var(--app-shadow)' }}>
          ...
          <Stack gap={4}>
            {(() => {
              ...
              return navItems.map((item, index) => {
                ...
                return (
                  <div key={item.path}>
                    {isNewSection && index > 0 && <Divider my={8} color="var(--app-border)" />}
                    {isNewSection && (
                    ...
                    <NavLink
                      ...
                      rightSection={item.badge ? <Badge size="sm" variant="filled" fw={600} style={{ backgroundColor: APP_ACCENT, color: '#ffffff' }}>{item.badge}</Badge> : null}
                      active={item.active}
                      ...
                      styles={{
                        root: {
                          ...
                          padding: '10px 16px',
                          marginBottom: '2px',
                          borderLeft: item.active ? `3px solid ${APP_ACCENT}` : '3px solid transparent',
                          transition: 'all 0.2s ease',
                          '&:hover': { backgroundColor: item.active ? 'var(--app-nav-active)' : 'var(--app-nav-hover)' },
                        },
                        label: { color: item.active ? APP_ACCENT : CHARCOAL, fontSize: '14px' },
                        section: { color: item.active ? APP_ACCENT : APP_ICON_MUTED },
                      }}
                      style={{ backgroundColor: item.active ? 'var(--app-nav-active)' : 'transparent' }}
                    />
                  </div>
                );
              });
        ...
        <Box px="lg" py="md" style={{ borderTop: '1px solid var(--app-border)' }}>
      ...
      <AppShell.Header style={{ backgroundColor: 'var(--app-header-bg)', borderBottom: '1px solid var(--app-border)' }}>
        <Flex align="center" justify="space-between" h="100%" px="lg">
          <Group gap="md">
            <Burger opened={opened} onClick={() => setOpened(o => !o)} hiddenFrom="sm" size="sm" color={APP_ACCENT} />
            <Group gap={10}>
              <img src="/sola_logo.png" alt="SOLA Logo" style={{ width: 34, height: 34, objectFit: 'contain' }} />
              <Box>
                <Text size="md" fw={700} c={APP_ACCENT} lh={1.2}>SOLA</Text>
            ...
            <Menu shadow="md" width={180} position="bottom-end">
              <Menu.Target>
                <Tooltip label="Account" position="bottom">
                  <ActionIcon size={36} variant="subtle" color="gray" radius="xl" p={0} style={{ color: APP_ICON_MUTED }}>
                    <UserAvatar src={userData?.profileImage} firstName={userData?.firstName} size={32} bg={APP_ACCENT} />
                  </ActionIcon>
                </Tooltip>
              </Menu.Target>
              <Menu.Dropdown style={{ borderRadius: '10px', border: '1px solid var(--app-border)' }}>
                ...
                <Menu.Item leftSection={<IconSettings size={16} />} onClick={() => navigate('/admin/settings')} styles={MENU_ITEM_STYLES}>
                  Settings
                </Menu.Item>
